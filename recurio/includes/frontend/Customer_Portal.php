<?php
namespace Recurio\Frontend;
use Recurio\Traits\Singleton;
use Recurio_Payment_Methods;
use Recurio_Subscription_Engine;
use Recurio_Billing_Manager;
...
class Customer_Portal {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_recurio_skip_subscription', array( $this, 'ajax_skip_subscription' ) );
			...
			if ( ! $subscription || get_current_user_id() != $subscription->customer_id ) {
				...
				$this->load_template(
					...
					array(
						...
						'pending_renewal_orders' => $this->get_pending_renewal_orders( $subscription_id ),
	...
	/**
	 * Get actionable pending/failed renewal orders for a subscription.
	 *
	 * Only returns orders where the customer still needs to pay:
	 *   1. New-style early renewal orders — created by the current code, which stores
	 *      _recurio_early_renewal_new_next_payment on the order and defers subscription
	 *      updates until payment. These are safe to surface for payment.
	 *   2. Regular (automated) renewal orders that failed payment — the subscription
	 *      date was not yet advanced, so payment is still required.
	 *
	 * Old-style early renewal orders (created before the deferred-update fix) are
	 * excluded because the subscription was already updated at click time — surfacing
	 * them would result in a double-charge for the same period.
	 *
	 * Orders already marked _recurio_renewal_processed = yes are also excluded.
	 *
	...
	private function get_pending_renewal_orders( $subscription_id ) {
		...
		$order_ids = $wpdb->get_col(
			$wpdb->prepare(
				...
				FROM {$wpdb->posts} p
				INNER JOIN {$wpdb->postmeta} pm_sub
					ON p.ID = pm_sub.post_id
					AND pm_sub.meta_key = '_recurio_subscription_id'
					AND pm_sub.meta_value = %s
				INNER JOIN {$wpdb->postmeta} pm_renewal
					ON p.ID = pm_renewal.post_id
					AND pm_renewal.meta_key = '_recurio_is_renewal_order'
					AND pm_renewal.meta_value = 'yes'
				LEFT JOIN {$wpdb->postmeta} pm_processed
					ON p.ID = pm_processed.post_id
					AND pm_processed.meta_key = '_recurio_renewal_processed'
					AND pm_processed.meta_value = 'yes'
				WHERE p.post_type = 'shop_order'
				AND p.post_status IN ('wc-pending', 'wc-failed')
				AND pm_processed.post_id IS NULL
				AND (
					EXISTS (
						SELECT 1 FROM {$wpdb->postmeta}
						WHERE post_id = p.ID
						AND meta_key = '_recurio_early_renewal_new_next_payment'
					)
					OR NOT EXISTS (
						SELECT 1 FROM {$wpdb->postmeta}
						WHERE post_id = p.ID
						AND meta_key = '_recurio_is_early_renewal'
						AND meta_value = 'yes'
					)
				)
		...
		return array_values( array_filter( array_map( 'wc_get_order', $order_ids ) ) );
	}
	...
	private function get_available_actions( $subscription ) {
		...
		$skip_cycle_enabled    = ! empty( $settings['general']['enable_skip_billing_cycle'] );
		...
		switch ( $subscription->status ) {
			case 'active':
				$actions[] = 'pause';
				if ( $skip_cycle_enabled && ! empty( $subscription->next_payment_date ) ) {
					$actions[] = 'skip_next_payment';
				}
				$actions[] = 'cancel';
	...
	public function ajax_cancel_subscription() {
		...
		/**
		 * Allow extensions (e.g. Recurio Pro) to run a retention flow before cancelling.
		 * Implementations must call wp_send_json_* + exit/die to respond; otherwise continue.
		 *
		 * @param int       $subscription_id Subscription ID (validated).
		 * @param string    $cancel_at       cancel_at preference (end_of_period|immediate).
		 * @param \stdClass $subscription    Subscription row.
		 */
		do_action( 'recurio_portal_before_subscription_cancel', $subscription_id, $cancel_at, $subscription );
	...
	/**
	 * Handle AJAX request to skip the next billing cycle
	 *
	 * @since 1.0.2
	 */
	public function ajax_skip_subscription() {
		check_ajax_referer( 'recurio_portal_nonce', 'nonce' );
		...
		if ( ! $skip_cycle_enabled ) {
			wp_send_json_error( __( 'Skipping a billing cycle is not enabled for your store.', 'recurio' ) );
		}
		...
		$engine = Recurio_Subscription_Engine::get_instance();
		$result = $engine->skip_next_cycle( $subscription_id );
